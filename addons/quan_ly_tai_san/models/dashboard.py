...
class AssetDashboard(models.Model):
    ...
    # Add a basic search view to prevent Odoo from trying to compute one
    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        """Override to handle search views for transient model"""
        result = super().fields_view_get(view_id, view_type, toolbar, submenu)
        if view_type == 'search' and not view_id:
            # Return a minimal search view for transient model
            result['arch'] = '''<search/>'''
        return result
